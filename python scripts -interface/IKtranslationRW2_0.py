...
if __name__ == "__main__":
    ...
    def func_x(x_val, y_val, z_val, a_val, b_val):
        return max(x_val + math.sin(math.radians(a_val)) * La + math.cos(math.radians(a_val)) * math.sin(math.radians(b_val)) * Lb,0)

    def func_y(x_val, y_val, z_val, a_val, b_val):
        return max(y_val - La + math.cos(math.radians(a_val)) * La - math.sin(math.radians(a_val)) * math.sin(math.radians(b_val)) * Lb,0)

    def func_z(x_val, y_val, z_val, a_val, b_val):
        return max(z_val + math.cos(math.radians(b_val)) * Lb - Lb,0)

    input_file = "BENTsilindir4.gcode"
    output_file = "BENTsilindir6_IK.gcode"
