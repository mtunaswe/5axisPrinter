...
@author: stefa
Modified by Janis Andersons to output B-axis coordinates (angle)
"""
...
INPUT_FILE_NAME = "silindir4.gcode"
OUTPUT_FILE_NAME = "BENT" + INPUT_FILE_NAME 
LAYER_HEIGHT = 0.28 #Layer height of the sliced gcode
WARNING_ANGLE = 100 #Maximum Angle printable with your setup
 
#2-point spline
#SPLINE_X = [40, 130]
##SPLINE_Z = [0, 130]
SPLINE_X = [115.5, 205.5]     # Follows the 90 mm arc from your base X
SPLINE_Z = [0, 100]          # Matches your cylinder height


SPLINE = CubicSpline(SPLINE_Z, SPLINE_X, bc_type=((1, 0), (1, 2.5))) #bent 90°
...
# Define the points at which you want to evaluate the spline
evaluation_points = np.linspace(min(SPLINE_Z), max(SPLINE_Z), 100)  # Adjust the number of points as desired

# Evaluate the spline at the evaluation points
x_coordinates = SPLINE(evaluation_points)
z_coordinates = evaluation_points

# Store the coordinates in a list
coordinates = list(zip(x_coordinates, z_coordinates))
...
def writeLine(G, X, Y, Z, A, F = None, E = None): #write a line to the output file
    outputSting = "G" + str(int(G)) + " X" + str(round(X,5)) + " Y" + str(round(Y,5)) + " Z" + str(round(Z,3)) + " A0 B" + str(round(A,3))
    if E is not None:
...
with open(INPUT_FILE_NAME, "r") as gcodeFile, open(OUTPUT_FILE_NAME, "w+") as outputFile:
        for currentLine in gcodeFile:
            ...
            if currentLineCommands is not None: #if current comannd is a valid gcode
                ...
                Baxis = angleSplineThisLayer * 57.2958 #radians to degrees
                writeLine(1,transformedGCode.y, currentPosition.y, transformedGCode.x, Baxis, None, extrusionAmount)
